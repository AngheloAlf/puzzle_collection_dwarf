/*
    Compile unit: C:\users\tsuji\title\src\panepon\gc\src\tetris.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x0009F174 -> 0x000A0B40
*/
int gCounter; // size: 0x4, address: 0xF4E80
int gReset; // size: 0x4, address: 0xDF598
int gMain; // size: 0x4, address: 0xDF59C
unsigned long long gTime; // size: 0x8, address: 0xF4E88
...
} * gAllVertex; // size: 0x4, address: 0xF4EA8
int gDemo; // size: 0x4, address: 0xF4EAC
int gWhatever; // size: 0x4, address: 0xF4EB0
int DEBUGDEBUG; // size: 0x4, address: 0xF4EB4
...
// Range: 0x9F174 -> 0x9F28C
void pon_InitStartPanel(struct tetWell * well /* r31 */, int num /* r1+0xC */) {
    // Local variables
    char pattern[18]; // r1+0x10

    // References
    // -> struct Game gTheGame;
}

// Range: 0x9F28C -> 0x9F2BC
void pon_InitgTheGame() {
    // References
    // -> struct Game gTheGame;
}

// Range: 0x9F2BC -> 0x9F378
int RandomBlock(struct tetWell * well /* r1+0x8 */) {
    // Local variables
    float rand; // f3
    int max; // r6
...
void InitBlockPattern(struct tetWell * well /* r28 */, char * ptr /* r30 */, int total /* r31 */, int clear /* r27 */) {
    // Local variables
    char * which; // r28
    int count; // r27
    int pos; // r3
    ...
    // References
    // -> char panel_data[14][6];
    // -> char clearpanel_data[20][6];
}

// Range: 0x9F490 -> 0x9F5DC
void InitStartingBlocks(struct tetWell * well /* r3 */, char * ptr /* r1+0xC */, int total /* r1+0x10 */) {
    // Local variables
    int row; // r12
    int col; // r31
...
// Range: 0x9F5DC -> 0x9F704
int DemoCheck(int * frame /* r30 */) {
    // References
    // -> int gDemo;
...
// Range: 0x9F704 -> 0x9F904
void DemoCPU(int num /* r30 */, int level /* r1+0xC */) {
    // References
...
int gShit; // size: 0x4, address: 0x116F70
int FRAME; // size: 0x4, address: 0x116F74
unsigned long gFrameColor; // size: 0x4, address: 0x4CFE8
char * Pon_Image_Heap; // size: 0x4, address: 0x184
struct player_t * gPlayer[4]; // size: 0x10, address: 0x116F78
unsigned char puzzle3D[]; // size: 0x0, address: 0xB553C
// Range: 0x9F904 -> 0xA037C
void InitTetrisWell() {
    // Local variables
    int num; // r21
    int flag; // r1+0x8
...
// Range: 0xA09F8 -> 0xA0B40
void HackGame(struct tetWell * well /* r3 */) {
    // Local variables
    int count; // r6
...
float CPUCPU; // size: 0x4, address: 0x116F8C
float RCPRCP; // size: 0x4, address: 0x116F90
int FrameRate; // size: 0x4, address: 0x116F94
int gFrame; // size: 0x4, address: 0x116F98

/*
    Compile unit: C:\users\tsuji\title\src\panepon\gc\src\main.c
...
struct OSThread_s {
    ...
    } context; // offset 0x20, size 0x190
};
...
} g_msg; // size: 0x2, address: 0x50F64
struct {
    // total size: 0x31B90
    struct {
        // total size: 0x31B88
...
} gInfo[2]; // size: 0x63720, address: 0x50F68
...
// Range: 0x65F70 -> 0x6622C
static unsigned long doMenuLoop(unsigned long drawbuffer /* r24 */) {
    // Local variables
    int loop; // r25
    ...
    // -> struct OSMesgQueue_s gSerialMsgQ;
    // -> int gCounter;
    // -> struct [anonymous] gInfo[2];
    // -> union [anonymous] g_msg;
...
// Range: 0x6622C -> 0x66424
static unsigned long doGameLoop(unsigned long drawbuffer /* r27 */) {
    // Local variables
    int loop; // r28
    int frame_count; // r1+0x10
    union {
...
// Range: 0x66424 -> 0x664D4
static void DoPadReset() {
    // Local variables
    int iUser; // r30
...
// Range: 0x664D4 -> 0x669AC
static int pon_DoDemoFromNPCMain() {
    // Local variables
    unsigned long drawbuffer; // r1+0x8
    int num; // r8
...
struct {
    ...
    struct flic_t flic[340]; // offset 0x2E848, size 0x2A80
    struct text_t drawText[70]; // offset 0x312C8, size 0x8C0
} dynamic; // size: 0x31B88, address: 0xB4688
...
void * dmaMessageBuf; // size: 0x4, address: 0xE624C
struct OSMesgQueue_s gDmaMsgQ; // size: 0x18, address: 0xE6250
unsigned long long bootStack[1024]; // size: 0x2000, address: 0xE6268

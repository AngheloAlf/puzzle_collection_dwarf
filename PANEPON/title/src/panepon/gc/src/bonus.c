/*
    Compile unit: C:\users\tsuji\title\src\panepon\gc\src\bonus.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x0003DB10 -> 0x00040CAC
*/
union_pon_gc_c_1 gCountDown[8]; // size: 0xC0, address: 0xAFA38
signed char gameoverBounce[40]; // size: 0x28, address: 0xAFAF8
...
// Range: 0x3DB10 -> 0x3DFC8
void InitBonus() {
    // Local variables
    int count; // r25
    int offset; // r1+0x8
    union_pon_gc_tex_c_12 * bg; // r3
    union_pon_gc_tex_c_7 * txtr; // r7
    char * currSegment; // r31
    int color8bit[4]; // r1+0xC8
    int size8bit[9][4]; // r1+0x38
    unsigned char FairyOffset[15][2]; // r1+0x18
    unsigned char BlockPosition[6]; // r1+0x10
...
void DoBonus() {
    ...
    int temp; // r4
    char * cont; // r28
    char string[5]; // r1+0x1B0
    struct tetWell * well; // r27
    int reboundXY[50][2]; // r1+0x20
...
// Range: 0x3E868 -> 0x3EB50
void DrawBonus(struct_pon_gc_draw_c_15 * dynamicp /* r30 */) {
    // Local variables
    int count; // r26
...
// Range: 0x3EB50 -> 0x3EB90
int DoWatchMenu() {
    // Local variables
    int temp; // r3
    unsigned short button; // r1+0x0
...
// Range: 0x3EB90 -> 0x3F4A8
void DoCountDown() {
    // Local variables
    struct tetWell * well; // r29
    struct cursor_t * cursor; // r31
    struct gamepad_t * gamepad; // r30
    unsigned short t_button; // r20
    ...
    // -> union [anonymous] gCountDown[8];
}

union_pon_gc_tex_c_7 countLUT[]; // size: 0x0, address: 0xDC028
union_pon_gc_tex_c_7 countTexture[]; // size: 0x0, address: 0xDC840
unsigned short countTable[]; // size: 0x0, address: 0xDBE28
unsigned char countdown[]; // size: 0x0, address: 0xDC040
// Range: 0x3F4A8 -> 0x3FBB4
void DrawCountDown() {
    // Local variables
    unsigned short tmem; // r1+0x8
    union_pon_gc_c_0 * _g; // r1+0x8
    union_pon_gc_c_0 * _g; // r1+0x8
    int num; // r25
    union_pon_gc_c_0 * _g; // r1+0x8
    ...
    // -> union [anonymous] countTexture[];
    // -> union [anonymous] countLUT[];
    // -> int gCounter;
}

static int gnTickClear; // size: 0x4, address: 0x1818
static int giScreenClear; // size: 0x4, address: 0x181C
static int gnTagTextClear; // size: 0x4, address: 0x1820
unsigned int FlicTable[340]; // size: 0x550, address: 0x50894
// Range: 0x3FBB4 -> 0x4078C
void InitStageClearIntro() {
    // Local variables
    int who; // r26
    int round; // r25
    int count; // r1+0x8
    int temp; // r7
    int offsetY[7]; // r1+0x14
    int f_bg; // r24
    int m_bg; // r23
    int s_bg; // r22
    ...
    // References
    // -> struct Game gTheGame;
    // -> static int giScreenClear;
    // -> static int gnTagTextClear;
    // -> static int gnTickClear;
    // -> char * nextSegment;
    // -> int gCounter;
    // -> int gWorld;
    // -> int gWhatever;
...
// Range: 0x40AA8 -> 0x40CAC
void DrawStageClearIntro(struct_pon_gc_draw_c_15 * dynamicp /* r30 */) {
    // Local variables
    int r; // r29
    int g; // r28
    int b; // r31

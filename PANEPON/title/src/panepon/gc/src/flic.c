/*
    Compile unit: C:\users\tsuji\title\src\panepon\gc\src\flic.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x0005CDFC -> 0x0005EDD4
*/
struct block_t {
    // total size: 0x2C
...
// Range: 0x5CDFC -> 0x5D080
void InitFlic() {
    // Local variables
    int i; // r30
    struct flic_t * frame; // r29
    struct action_t * action; // r4
...
// Range: 0x5D080 -> 0x5D150
void InitTrack(char * currSegment /* r31 */, int first /* r26 */, int color /* r5 */, int bg /* r27 */, int text /* r28 */, int sprite /* r29 */, int track /* r30 */) {
    // Local variables
    int offset; // r4
    int base; // r4
}

// Range: 0x5D150 -> 0x5D20C
void pon_InitTrack(char * currSegment /* r29 */, int count /* r30 */, int color /* r5 */, int bg /* r26 */, int text /* r27 */, int sprite /* r28 */, int track /* r31 */) {
    // Local variables
    int offset; // r4
    int base; // r4
}

// Range: 0x5D20C -> 0x5D2B4
int InitAnimeColor(char * currSegment /* r3 */, int base /* r1+0x4 */, int color /* r1+0x8 */) {
    // Local variables
    int i; // r1+0x0
    int offset; // r7
    signed short * infor; // r8
    union_pon_gc_tex_c_7 * lut; // r9

    // References
    // -> struct Game gTheGame;
}

// Range: 0x5D2B4 -> 0x5D3CC
int InitAnimeBG(char * currSegment /* r26 */, int offset /* r27 */, int base /* r5 */, int count /* r1+0x14 */) {
    // Local variables
    int i; // r29
    signed short * infor; // r28
    union_pon_gc_tex_c_12 * bg; // r3

    // References
    // -> struct Game gTheGame;
}

// Range: 0x5D3CC -> 0x5D558
void InitAnimeBGTransparent() {
    // Local variables
    int i; // r7
    struct flic_t * frame; // r8

    // References
    // -> struct Game gTheGame;
}

// Range: 0x5D558 -> 0x5D728
int InitAnimeText(char * currSegment /* r1+0x8 */, int offset /* r31 */, int base /* r1+0x10 */, int count /* r1+0x14 */) {
    // Local variables
    int i; // r1+0x8
    signed short * infor; // r11
    union_pon_gc_tex_c_7 * txtr; // r12

    // References
    // -> struct Game gTheGame;
}

// Range: 0x5D728 -> 0x5D834
int InitAnimeSprite(char * currSegment /* r1+0x8 */, int offset /* r3 */, int base /* r5 */, int count /* r1+0x14 */) {
    // Local variables
    int i; // r1+0x8
    int j; // r12
    int total; // r5
    signed short * infor; // r6
    union_pon_gc_c_1 * sp; // r31

    // References
    // -> struct Game gTheGame;
}

// Range: 0x5D834 -> 0x5D8D0
void InitAnimeFlic(char * currSegment /* r1+0x0 */, int offset /* r1+0x4 */, int base /* r5 */, int track /* r6 */) {
    // Local variables
    int i; // r1+0x0
    int j; // r4
    int total; // r5
    signed short * infor; // r7
    struct flic_t * frame; // r8
...
// Range: 0x5DAB4 -> 0x5E180
void SortFlic() {
    // Local variables
    int i; // r10
    int j; // r25
    struct flic_t temp; // r1+0x560
    struct flic_t * flic1; // r21
    struct flic_t * flic2; // r20
    unsigned int what; // r3
    unsigned int array[340]; // r1+0x10
...
// Range: 0x5E180 -> 0x5E354
void UpdateFlic(int last /* r1+0x8 */) {
    // Local variables
    int i; // r1+0x8
    int array[340]; // r1+0x10
    int add; // r5
    struct flic_t * frame; // r8
...
// Range: 0x5E354 -> 0x5ED94
void Draw2DAnimation(struct_pon_gc_draw_c_15 * dynamicp /* r1+0xB54 */, int from /* r16 */, int to /* r1+0xB58 */) {
    // Local variables
    int i; // r7
    int layer; // r16
    int reset; // r25
    int addBG; // r19
    int drawBG[200]; // r1+0x834
    int fadeBG[200]; // r1+0x514
    int addSP; // r18
    int drawSP[160]; // r1+0x294
    int fadeSP[160]; // r1+0x14
    struct flic_t * frame; // r3

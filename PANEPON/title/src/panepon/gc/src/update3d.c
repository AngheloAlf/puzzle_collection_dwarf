/*
    Compile unit: C:\users\tsuji\title\src\panepon\gc\src\update3d.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x000AE6D4 -> 0x000B03D8
*/
signed char block3Dview[8][12]; // size: 0x60, address: 0xE0CC0
...
// Range: 0xAE6D4 -> 0xAE7A4
void Update3DDistance(struct tetWell * well /* r1+0x8 */, struct cursor_t * cursor /* r1+0xC */) {
    // Local variables
    int distance; // r1+0x8
    struct attack_t * attack; // r5

    // References
    // -> int gSelection;
}

// Range: 0xAE7A4 -> 0xAF0C8
void Update3DPosition(struct tetWell * well /* r31 */, int flag /* r1+0x10 */) {
    // Local variables
    int row; // r28
    int count; // r1+0x8
    struct block_t last[12]; // r1+0x1C
    struct attack_t * attack; // r8
    struct explode_t * explode; // r7
}

// Range: 0xAF0C8 -> 0xAF130
void Update3DCursor(struct tetWell * well /* r3 */, struct cursor_t * cursor /* r4 */) {}
...
// Range: 0xAF130 -> 0xAF670
void Update3DIcon(struct tetWell * well /* r18 */, struct cursor_t * cursor /* r19 */, int num /* r20 */) {
    // Local variables
    int i; // r5
    int temp; // r4
    int two; // r23
...
// Range: 0xAF670 -> 0xB03D8
void Check3DVisibleBlocks(struct tetWell * well /* r30 */, struct cursor_t * cursor /* r31 */) {
    // Local variables
    int row; // r12
    int col; // r11
    int temp; // r1+0x8

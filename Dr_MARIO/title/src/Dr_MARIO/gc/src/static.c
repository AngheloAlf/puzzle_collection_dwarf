/*
    Compile unit: C:\users\tsuji\title\src\Dr_MARIO\gc\src\static.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x00056DB0 -> 0x00057228
*/
union_drmario_gc_c_0 S2RDPinit_dl[13]; // size: 0x68, address: 0x24938
union_drmario_gc_c_0 S2ClearCFB_dl[5]; // size: 0x28, address: 0x249A0
union_drmario_gc_c_0 S2Spriteinit_dl[9]; // size: 0x48, address: 0x249C8
union_drmario_gc_c_0 F3RDPinit_dl[28]; // size: 0xE0, address: 0x24A10
union_drmario_gc_c_0 F3SetupRDP_dl[4]; // size: 0x20, address: 0x24AF0
union_drmario_gc_c_0 F3SetupRSP_dl[7]; // size: 0x38, address: 0x24B10
union_drmario_gc_c_0 * gp; // size: 0x4, address: 0x1E9A4
// Range: 0x56DB0 -> 0x56E60
void load_TexPal(void * pal /* r1+0x0 */) {
    // Local variables
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0

    // References
    // -> union [anonymous] * gp;
}

// Range: 0x56E60 -> 0x56FB4
void load_TexBlock_4b(void * grp /* r1+0x0 */, int size_w /* r1+0x4 */, int size_h /* r1+0x8 */) {
    // Local variables
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0

    // References
    // -> union [anonymous] * gp;
}

// Range: 0x56FB4 -> 0x570D8
void load_TexTile_4b(void * grp /* r1+0x8 */, int size_w /* r1+0xC */, int start_w /* r1+0x14 */, int start_h /* r1+0x18 */, int end_w /* r1+0x1C */, int end_h /* r1+0x20 */) {
    // Local variables
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8
    union_drmario_gc_c_0 * _g; // r1+0x8

    // References
    // -> union [anonymous] * gp;
}

// Range: 0x570D8 -> 0x57228
void draw_Tex(int x_pos /* r1+0x0 */, int y_pos /* r1+0x4 */, int draw_w /* r1+0x8 */, int draw_h /* r1+0xC */, int start_w /* r1+0x10 */, int start_h /* r1+0x14 */) {
    // Local variables
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0
    union_drmario_gc_c_0 * _g; // r1+0x0

    // References
    // -> union [anonymous] * gp;
}



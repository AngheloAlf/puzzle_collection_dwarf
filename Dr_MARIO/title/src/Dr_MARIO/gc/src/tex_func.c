...
*/
union_drmario_gc_c_0 copy_texture_init_dl[14]; // size: 0x70, address: 0x4130
union_drmario_gc_c_0 alpha_texture_init_dl[13]; // size: 0x68, address: 0x41A0
union_drmario_gc_c_0 normal_texture_init_dl[14]; // size: 0x70, address: 0x4208
static int _modes$96[4]; // size: 0x10, address: 0x120
static union_drmario_gc_c_0 init_dl$155[5]; // size: 0x28, address: 0x4278
struct SCopyTexBlock {
    // total size: 0x28
    union_drmario_gc_c_0 * * gpp; // offset 0x0, size 0x4
...
struct SStretchTexBlock {
    // total size: 0x6C
    union_drmario_gc_c_0 * * gpp; // offset 0x0, size 0x4
    union_tex_func_c_7 * * vpp; // offset 0x4, size 0x4
...
// Range: 0x52F4 -> 0x53D4
struct_tex_func_c_8 * tiLoadTexData(void * hpp /* r30 */, void * romStart /* r3 */, void * romEnd /* r5 */) {
    // Local variables
    void * hp; // r31
    struct_tex_func_c_8 * texInfo; // r3
    int texSize; // r1+0x8
}

static unsigned char _pnts$871[9][8]; // size: 0x48, address: 0x148
// Range: 0x53D4 -> 0x56E0
void drawCursorPattern(union_drmario_gc_c_0 * * gpp /* r1+0x8 */, int frmW /* r1+0x14 */, int frmH /* r1+0x18 */, int posX /* r1+0x1C */, int posY /* r1+0x20 */, int posW /* r1+0x24 */, int posH /* r8 */) {
    // Local variables
    union_drmario_gc_c_0 * gp; // r5
    int px[4]; // r1+0x60
    int py[4]; // r1+0x50
    int tx[3]; // r1+0x44
    int ty[3]; // r1+0x38
